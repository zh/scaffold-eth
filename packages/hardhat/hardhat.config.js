...
module.exports = {
  ...
  namedAccounts: {
    ...
    mainnet: deployerAddress,
    fuji: deployerAddress,
  },
};
...
task("balances", "Prints the balance of all accounts", async (_, { ethers }) => {
  const accounts = await ethers.provider.listAccounts();
  await Promise.all(
    accounts.map(async (account) => {
      const address = await addr(ethers, account);
      const balance = await ethers.provider.getBalance(address);
      console.log(account + ": " + formatUnits(balance, "ether"), "AVAX");
    })
  );
});
